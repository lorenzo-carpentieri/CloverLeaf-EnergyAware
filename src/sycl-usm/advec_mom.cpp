...
void advec_mom_kernel(synergy::queue &queue, int x_min, int x_max, int y_min, int y_max, clover::Buffer2D<double> vel1,
                      ...
                      clover::Buffer2D<double> mom_flux, clover::Buffer2D<double> pre_vol, clover::Buffer2D<double> post_vol,
                      clover::Buffer1D<double> celldx, clover::Buffer1D<double> celldy, int which_vel, int sweep_number, int direction) {
  ...
  if (direction == 1) {
    if (which_vel == 1) {
      ...
      clover::par_ranged2(queue, Range2d{x_min - 1 + 1, y_min + 1, x_max + 2 + 2, y_max + 1 + 2}, [=](const int i, const int j) {
    ...
    clover::par_ranged2(queue, Range2d{x_min - 1 + 1, y_min + 1, x_max + 1 + 2, y_max + 1 + 2}, [=](const int x, const int y) {
      ...
      if (vdiffuw * vdiffdw > 0.0) {
        ...
        limiter = wind * sycl::fmin(sycl::fmin(width * ((2.0 - sigma) * adw / width + (1.0 + sigma) * auw / celldx[dif]) / 6.0, auw), adw);
      }
    ...
    if (which_vel == 1) {
      ...
      clover::par_ranged2(queue, Range2d{x_min + 1, y_min - 1 + 1, x_max + 1 + 2, y_max + 2 + 2}, [=](const int i, const int j) {
        node_mass_post(i, j) = 0.25 * (density1(i + 0, j - 1) * post_vol(i + 0, j - 1) + density1(i, j) * post_vol(i, j) +
                                       density1(i - 1, j - 1) * post_vol(i - 1, j - 1) + density1(i - 1, j + 0) * post_vol(i - 1, j + 0));
    ...
    clover::par_ranged2(queue, Range2d{x_min + 1, y_min - 1 + 1, x_max + 1 + 2, y_max + 1 + 2}, [=](const int x, const int y) {
      int upwind, donor, downwind, dif;
      double sigma, width, limiter, vdiffuw, vdiffdw, auw, adw, wind, advec_vel_s;
      ...
      if (vdiffuw * vdiffdw > 0.0) {
        ...
        wind = 1.0;
        if (vdiffdw <= 0.0) wind = -1.0;
        limiter = wind * sycl::fmin(sycl::fmin(width * ((2.0 - sigma) * adw / width + (1.0 + sigma) * auw / celldy[dif]) / 6.0, auw), adw);
      }
...
void advec_mom_driver(global_variables &globals, int tile, int which_vel, int direction, int sweep_number) {
  ...
  if (which_vel == 1) {
    advec_mom_kernel(globals.context.queue, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin, t.info.t_ymax, t.field.xvel1, t.field.mass_flux_x,
                     t.field.vol_flux_x, t.field.mass_flux_y, t.field.vol_flux_y, t.field.volume, t.field.density1, t.field.work_array1,
                     t.field.work_array2, t.field.work_array3, t.field.work_array4, t.field.work_array5, t.field.work_array6,
                     t.field.celldx, t.field.celldy, which_vel, sweep_number, direction);
  } else {
    advec_mom_kernel(globals.context.queue, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin, t.info.t_ymax, t.field.yvel1, t.field.mass_flux_x,
                     t.field.vol_flux_x, t.field.mass_flux_y, t.field.vol_flux_y, t.field.volume, t.field.density1, t.field.work_array1,
                     t.field.work_array2, t.field.work_array3, t.field.work_array4, t.field.work_array5, t.field.work_array6,
                     t.field.celldx, t.field.celldy, which_vel, sweep_number, direction);
  }
